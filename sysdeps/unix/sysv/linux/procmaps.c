...
/* Parse the leading "<start>-<end> <perms>" fields of a /proc/self/maps line
   and pass them to the user provided closure.  */
static bool
...
enum
{
  /* Each /proc/self/maps line starts with "<start>-<end> <perms> ", where
     START and END are hexadecimal numbers and PERMS is a 4 character string.
     The parse_line only parses these leading fields and the reader passes
     longer lines truncated, discarding the rest of the line (such as the
     pathname).  */
