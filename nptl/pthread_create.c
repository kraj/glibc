...
__pthread_init_stack (struct pthread *result)
{
  ...
  /* getrandom_buf, timerid and timer_overrun are intentionally not reset
     ...
     - timerid and timer_overrun: meaningful only for SIGEV_THREAD helper
       ...
       helper from its setup barrier; timer_overrun is re-seeded at the start
       of every notification and must survive across a cancelled or exited
       notification so timer_getoverrun can still read it.  */
