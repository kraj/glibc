...
#if _STACK_GROWS_DOWN
      /* /proc might not be accessible, for instance a chroot without it
	 mounted.  The kernel copies the AT_EXECFN string at the very
	 top of the argument area, so it provides an estimate of the top
	 of the stack vma without /proc/self/maps.  The clamp against
	 the mapping below the stack is not possible in this case, so an
	 unlimited RLIMIT_STACK can not be bounded.  */
